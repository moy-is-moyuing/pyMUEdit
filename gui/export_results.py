...
from PyQt5.QtWidgets import (
    ...
    QStackedWidget,
    QFileDialog,
    QMessageBox,  # Import needed dialogs
)
...
class ExportSetupWidget(QWidget):
    ...
    def _create_export_setup_section(self):
        ...
        format_label.setStyleSheet(f"color: {self.colors['text_primary']}; margin-bottom: -5px;")
        card_layout.addWidget(format_label)
        self.format_combo = QComboBox()
        self.format_combo.addItems(
            [".csv (Comma Separated Values)", ".mat (MATLAB)", ".xlsx (Excel Spreadsheet)", ".txt (Text File)"]
        )
        self.format_combo.setPlaceholderText("Choose an export format...")
        ...
        data_title_label.setStyleSheet(f"color: {self.colors['text_primary']};")
        data_details_layout.addWidget(data_title_label)
        data_details_layout.addWidget(
            self._create_info_item(get_icon(QStyle.SP_FileIcon), "Motor Unit Firing Patterns")
        )  # Uses get_icon
        data_details_layout.addWidget(
            self._create_info_item(get_icon(QStyle.SP_DialogApplyButton), "Recording Duration: 120s")
        )  # Uses get_icon
        data_details_layout.addWidget(
            self._create_info_item(get_icon(QStyle.SP_ComputerIcon), "Units Detected: 12")
        )  # Uses get_icon
        card_layout.addWidget(data_details_frame)
        export_button = QPushButton("Export Data")
        export_button.setFont(QFont("Arial", 10, QFont.Bold))
        export_button.setIcon(get_icon(QStyle.SP_ArrowDown))
        export_button.setIconSize(QSize(16, 16))
        export_button.setMinimumHeight(40)
        export_button.setCursor(Qt.PointingHandCursor)  # Uses get_icon
        export_button.setStyleSheet(
            f""" QPushButton {{ background-color: {self.colors['button_dark_bg']}; color: {self.colors['button_dark_text']}; border: none; border-radius: 4px; padding: 8px 15px; }} QPushButton:hover {{ background-color: {self.colors['button_dark_hover']}; }} """
        )
    ...
    def _create_info_item(self, icon, text):
        ...
        text_label.setStyleSheet(f"color: {self.colors['text_secondary']};")
    ...
    def _create_recent_exports_section(self):
        ...
        recent_card.setStyleSheet(
            f""" #recentCardSetup {{ background-color: {self.colors['bg_card']}; border: 1px solid {self.colors['border_light']}; border-radius: 8px; padding: 15px; }} #recentCardSetup > QLabel {{ color: {self.colors['text_primary']}; font-size: 11pt; font-weight: bold; border: none; background: transparent; margin-bottom: 5px; }} """
        )
        ...
        recent_exports_data = [
            {
                "icon": get_icon(QStyle.SP_DialogHelpButton),
                "filename": "firing_patterns_2025.01.15.csv",
                "metadata": "2.4 MB • Completed",
            },
            {
                "icon": get_icon(QStyle.SP_DriveHDIcon),
        ...
        if not recent_exports_data:
            ...
            no_exports_label.setStyleSheet(
                f"color: {self.colors['text_secondary']}; font-style: italic; padding: 10px;"
            )
            no_exports_label.setAlignment(Qt.AlignCenter)
            card_layout.addWidget(no_exports_label)
        else:
            for export_data in recent_exports_data:
                card_layout.addWidget(
                    self._create_recent_export_item(
                        export_data["icon"], export_data["filename"], export_data["metadata"]
                    )
                )
        return recent_card
    ...
    def _handle_download_recent(self):
        ...
        if sender_button:
            ...
            print(f"Setup Widget: Download requested for recent file: {filename}")  # Add logic here
        else:
...
class ExportResultsWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.main_title_label = QLabel("Export Results")
        self.main_title_label.setFont(QFont("Arial", 16, QFont.Bold))
        self.main_title_label.setStyleSheet("color: #333333; border: none;")
        self.main_subtitle_label = QLabel("Export your motor unit firing patterns data")
        ...
        if self.confirmation_view:
            ...
            self.stacked_widget.addWidget(QLabel("Error: Confirmation view failed."))

        # Add complete view to stack
        if self.complete_view:
            self.stacked_widget.addWidget(self.complete_view)
        else:
            self.stacked_widget.addWidget(QLabel("Error: Complete view failed."))

        # --- Footer ---
        self.footer_label = QLabel("Last export: January 15, 2025 at 14:30")
