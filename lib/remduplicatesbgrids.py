...
def remduplicatesbgrids(PulseT, distime, muscle, maxlag, jitter, tol, fsamp):
    print("Removing duplicates between grids")
    ...
    for i in range(PulseT.shape[0]):
        ...
        for j in range(1, jit + 1):
            tmp_times.extend(distime[i] - j)
            tmp_times.extend(distime[i] + j)

        tmp_times.extend(distime[i])
    ...
    while len(distimmp) > 0:
        ...
        musclenew = np.append(musclenew, muscle[duplicates[survivor]])
        ...
        muscle = np.delete(muscle, duplicates)

        print(f"{len(distime)} remaining MUs to check")
        i += 1

    return Pulsenew, distimenew, musclenew
