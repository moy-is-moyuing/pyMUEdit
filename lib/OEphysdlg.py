...
class ColoredCircle(QWidget):
    """Widget that mimics MATLAB's Lamp component"""
...
class OEphysdlg(QMainWindow):
    """Python equivalent of MATLAB's OEphysdlg class"""
    ...
    def initUI(self):
        ...
        image_label = QLabel()
        image_path = os.path.join(path_to_app, "OpenEphys.jpg")
        ...
        # Add lamps to layout in the proper positions matching the image
        # (Adjusted from previous version to match OEphysdlg positions)
        lamps_layout = QHBoxLayout()
        lamps_layout.addWidget(self.lamp_A1)
        lamps_layout.addWidget(self.lamp_A2)
        lamps_layout.addWidget(self.lamp_B1)
        lamps_layout.addWidget(self.lamp_B2)
        lamps_layout.addWidget(self.lamp_C1)
        lamps_layout.addWidget(self.lamp_C2)
        lamps_layout.addWidget(self.lamp_D1)
        lamps_layout.addWidget(self.lamp_D2)

        indicators_layout.addLayout(lamps_layout)
        ...
        din_ain_layout = QHBoxLayout()
        din_ain_layout.addWidget(self.edit_field_Din)
        din_ain_layout.addWidget(self.edit_field_Ain)

        metrics_layout.addLayout(din_ain_layout)
        ...
        # Port panels section
        # First row of panels
        grid_panels_layout1 = QHBoxLayout()

        # Create port panels (now matching OEphysdlg order)
        ...
        grid_panels_layout1.addWidget(self.port_A1_panel)
        grid_panels_layout1.addWidget(self.port_B1_panel)
        grid_panels_layout1.addWidget(self.port_C1_panel)
        grid_panels_layout1.addWidget(self.port_D1_panel)

        main_layout.addLayout(grid_panels_layout1)
        ...
        # Add checkboxes to the appropriate locations near the panels
        self.port_A1_panel.layout().addWidget(self.checkbox_A1)  # type: ignore
        self.port_B1_panel.layout().addWidget(self.checkbox_B1)  # type: ignore
        self.port_C1_panel.layout().addWidget(self.checkbox_C1)  # type: ignore
        self.port_D1_panel.layout().addWidget(self.checkbox_D1)  # type: ignore

        self.port_A2_panel.layout().addWidget(self.checkbox_A2)  # type: ignore
        self.port_B2_panel.layout().addWidget(self.checkbox_B2)  # type: ignore
        self.port_C2_panel.layout().addWidget(self.checkbox_C2)  # type: ignore
        self.port_D2_panel.layout().addWidget(self.checkbox_D2)  # type: ignore
    ...
    def checkbox_A1_value_changed(self, state):
        """Handle checkbox A1 state change"""
    ...
    def checkbox_A2_value_changed(self, state):
        """Handle checkbox A2 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_B1_value_changed(self, state):
        """Handle checkbox B1 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_B2_value_changed(self, state):
        """Handle checkbox B2 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_C1_value_changed(self, state):
        """Handle checkbox C1 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_C2_value_changed(self, state):
        """Handle checkbox C2 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_D1_value_changed(self, state):
        """Handle checkbox D1 state change"""
        if state == Qt.CheckState.Checked:
    ...
    def checkbox_D2_value_changed(self, state):
        """Handle checkbox D2 state change"""
        if state == Qt.CheckState.Checked:
